...
- (instancetype)initWithData:(NSData *)data {
    do {
        ...
        if (![responseList isKindOfClass:NSArray.class] || responseList.count == 0)
            break;
        ...
        if (![resultSubdivisionDetails isKindOfClass:NSArray.class] || resultSubdivisionDetails.count == 0)
            break;

        NSDictionary *subdivisionValues = resultSubdivisionDetails[0];
        if (![subdivisionValues isKindOfClass:NSDictionary.class])
            break;

        NSDictionary *countryDetails = responseValues[@"country"];
        if (![countryDetails isKindOfClass:NSDictionary.class])
            break;
        ...
        if (![location isKindOfClass:NSString.class] || !location.length ||
            ![country isKindOfClass:NSString.class] || !country.length)
            break;
        /*
		 * City is valid.
		 */
...
+ (void)fetch {
    ...
    if (locationURLString.length) {
        locationURL = [NSURL URLWithString:locationURLString];
    }
#if DEBUG
    ...
    [[session dataTaskWithURL:locationURL
